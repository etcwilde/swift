...
void SILGenFunction::emitArtificialTopLevel(Decl *mainDecl) {
  ...
  case ArtificialMainKind::TypeMain: {
    ...
    if (!mainFunc->hasAsync()) {
      ...
      ModuleDecl *concShimsModule = getASTContext().getModuleByIdentifier(
          getASTContext().getIdentifier("_SwiftConcurrencyShims"));

      assert(concShimsModule && "Failed to load concurrency shims");

      FuncDecl *exitFuncDecl = nullptr;
      {
        SmallVector<ValueDecl *, 1> decls;
        concShimsModule->lookupQualified(
            concShimsModule, DeclNameRef(getASTContext().getIdentifier("exit")),
            NL_QualifiedDefault | NL_IncludeUsableFromInline, decls);

        if (decls.size() == 1)
          exitFuncDecl = dyn_cast<FuncDecl>(decls[0]);
      }
...
void SILGenFunction::emitAsyncMainThreadStart(SILDeclRef entryPoint) {
  ...
  auto createExplodyCastForCall = [this, &moduleLoc](SILValue originalValue,
                                         FuncDecl *jobRunFuncDecl,
    ...
    SILType apiSILType = SILType::getPrimitiveObjectType(apiType->getCanonicalType());
  ...
  SILValue taskFlags = emitWrapIntegerLiteral(moduleLoc, getLoweredType(ctx.getIntType()),
